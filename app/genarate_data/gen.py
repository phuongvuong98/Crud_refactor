...
def insert_items(items):
    query = "INSERT INTO product_variant(id, price, product_id, store_id, color_id) " \
            "VALUES(%s,%s,%s,%s,%s)"
    # query = "INSERT INTO brand(id, name) " \
    #         "VALUES(%s,%s)"
    # query = "INSERT INTO product(id, name, category_id) " \
    #         "VALUES(%s,%s,%s)"

    # try:
    db = MySQLdb.connect(host="localhost", user="root", passwd="", db="test", charset="utf8mb4", use_unicode=True)
    cursor = db.cursor()
    ...
    with open(os.path.abspath(os.path.dirname(__file__)) + '/variant.csv') as csv_file:
        ...
        for row in csv_reader:
            if line_count == 0:
                ...
                # names.append((str(line_count), row[0] + " " + str(line_count)))
                # names.append((str(line_count), row[0] + " " + str(line_count), str(line_count)))
                names.append((str(line_count), row[0], str(line_count),str(line_count),str(line_count)))
                line_count += 1
    print(names)
    insert_items(names)
